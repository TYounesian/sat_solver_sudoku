...
def dpll(cnf, heuristic):
    ...
    while True:
        ...
        cls.extend([x for x in unitClauses])                   # Save only the positive predicates
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 1:
        ...
        try:
            ...
            sys.exit("ERROR: Not valid rule!")
        try:
            puzzlesource = sys.argv[3]
        except:
            sys.exit("ERROR: Enter a path for a puzzle")
    else:
        rules = "etc/sudoku-rules-9x9.txt"
        heuristic = heuristics[heuristicName]
        puzzlesource = "etc/1000 sudokus.txt"
    ...
    solution = []
    heur_solution = []
    solutions = solver_complete(getRules(rules) + puzzles[0], heuristic)

#  =================================== WRITE DIMACS ===================================
    solution_file = open("solutions/dpll.txt",mode="w",encoding="utf-8")
    solution_file.write("p cnf {} {}\n".format(len(solutions), len(solutions)))
    for clause in solutions:
        solution_file.write("{} 0\n".format(clause))
    solution_file.write("Writing to a new file\n")
    solution_file.close()
